    ...
    <div class="bg-white rounded-2xl w-full max-w-lg p-6 border border-slate-200">
      ...
      <form id="serviceForm" class="space-y-4">
        ...
        <div>
          ...
          <input id="servicePrice" data-mask="brl" inputmode="numeric" placeholder="R$ 0,00" class="mt-2 w-full rounded-lg border border-slate-200 px-3 py-2" />
        </div>
        <div>
    ...
    const renderServices = (rows) => {
      const html = rows.map((service) => `
        <tr class="border-t border-slate-100">
          ...
          <td class="py-3">R$ ${(Number(service.price || 0) / 100).toFixed(2).replace('.', ',')}</td>
